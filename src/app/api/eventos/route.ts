...
import { SCENARIOS, validNumber } from "@/lib/validation";
...
const PASO_MS = 5_000; // Same cadence as the browser; response range is bounded.

export function GET(req: NextRequest) {
  const p = req.nextUrl.searchParams;
  const horas = validNumber(p.get("horas"), 4, 0.5, 24);
  const escenario = (p.get("escenario") ?? "clase_normal") as Escenario;
  if (horas === null || !SCENARIOS.includes(escenario)) {
    return NextResponse.json(
      { error: "Horas o escenario inválido." },
      { status: 400 },
    );
  }
  ...
  for (
    let t = Math.floor(desde / PASO_MS) * PASO_MS;
    t <= hasta;
    t += PASO_MS
  ) {
    const fecha = new Date(t);
    for (const aula of AULAS.map((a) => a.codigo as AulaCodigo)) {
      for (const m of medicionesEnTick(
        aula,
        escenario,
        HORARIO_DEFAULT,
        fecha,
      )) {
        for (const e of engine.process(m))
          if (e.accion !== "cerrar") eventos.push(e.evento);
      }
    }
    for (const e of engine.tick(t))
      if (e.accion !== "cerrar") eventos.push(e.evento);
  }
  return NextResponse.json({
    horas,
