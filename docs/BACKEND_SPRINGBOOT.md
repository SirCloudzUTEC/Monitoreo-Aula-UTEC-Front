...
- **Ventana de gracia del refresh**: un refresh token recién rotado que reaparece dentro de `REFRESH_GRACE_SECONDS` (30 s por defecto) no se toma por robo —dos pestañas que restauran la sesión a la vez envían la misma cookie, y una respuesta perdida provoca un reintento—; se emite un token nuevo sin revocar la familia. Pasada la ventana, el reuso sí revoca todas las sesiones. Un token revocado por logout, suspensión o cambio de contraseña nunca entra en la gracia.
